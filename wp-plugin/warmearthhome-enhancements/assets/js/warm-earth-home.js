/**
 * Warm Earth Home - Custom JavaScript (Plugin bundled fallback)
 * Version: 1.0.0
 */
(function($) {
	'use strict';
	$(document).ready(function() {
		function initHeroSlider() {
			...
			var autoSlide = setInterval(function() { nextSlide(); }, slideInterval);
			$slider.on('mouseenter', function() { clearInterval(autoSlide); }).on('mouseleave', function() {
				autoSlide = setInterval(function() { nextSlide(); }, slideInterval);
			});
			function nextSlide() {
		...
		function initQuickView() {
			$('.weh-quick-view-btn').on('click', function(e) {
				...
				if (typeof gtag !== 'undefined') {
					gtag('event', 'quick_view_open', {'product_id': productId});
				}
			});
		}
		...
		function initEmptyState() {
			...
			if ($productGrid.length > 0 && $productGrid.children().length === 0) {
				...
				if (typeof gtag !== 'undefined') {
					gtag('event', 'empty_state_view', { 'filter_combination': window.location.search });
				}
			}
		}
		...
		function initScrollToTop() {
			...
			$(window).scroll(function() {
				if ($(window).scrollTop() > 600) { $scrollBtn.fadeIn(); } else { $scrollBtn.fadeOut(); }
			});
			$scrollBtn.on('click', function() { $('html, body').animate({ scrollTop: 0 }, 600); });
		}
		function initFAQAccordion() {
			$('.weh-faq-question').on('click', function() {
				...
				if (isExpanded) {
					...
					if (typeof gtag !== 'undefined') {
						gtag('event', 'faq_open', { 'category': category, 'question': question });
					}
				}
			});
		}
		...
		function initContactForm() {
			$('.weh-contact-form').on('submit', function(e) {
				...
				$form.find('input[required], select[required], textarea[required]').each(function() {
					var $field = $(this);
					if (!$field.val().trim()) { isValid = false; $field.addClass('error'); } else { $field.removeClass('error'); }
				});
				var $email = $form.find('input[type="email"]');
				if ($email.length && $email.val()) {
					var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
					if (!emailRegex.test($email.val())) { isValid = false; $email.addClass('error'); }
				}
				if (!isValid) { e.preventDefault(); alert('Please fill in all required fields correctly.'); return false; }
				var issueType = $form.find('#contact-issue').val();
				if (typeof gtag !== 'undefined') { gtag('event', 'contact_submit', { 'issue_type': issueType || 'other' }); }
			});
		...
		function initScheduleCallTracking() {
			$('a[href*="Styling Consultation"]').on('click', function() {
				if (typeof gtag !== 'undefined') { gtag('event', 'schedule_call', { 'source': 'support_page' }); }
			});
		}
		function initOrderTracking() {
			$('a[href*="myaccount"]').on('click', function() {
				if (typeof gtag !== 'undefined') { gtag('event', 'order_track_click', { 'order_status': 'unknown' }); }
			});
		}
